...
const NFAAcquisitionDisposalsPendingIndex = async ({
	params,
	searchParams,
}) => {
	...
	const acquireIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/acquisitionsdisposals/pending?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default NFAAcquisitionDisposalsPendingIndex;
