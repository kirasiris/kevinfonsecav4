...
async function getCompanies(params) {
	const res = await fetchurl(
		`/global/companies${params}&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCompaniesDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/companies/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
