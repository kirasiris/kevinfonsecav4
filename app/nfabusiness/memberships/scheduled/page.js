...
async function getMembershipsPublished(params) {
	const res = await fetchurl(
		`/global/memberships${params}&status=scheduled`,
		"GET",
		"no-cache"
	);
	return res;
...
const NFAMembershipsScheduledIndex = async ({ params, searchParams }) => {
	...
	const activateIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/memberships/scheduled?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default NFAMembershipsScheduledIndex;
