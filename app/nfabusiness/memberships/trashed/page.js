...
async function getMembershipsPublished(params) {
	const res = await fetchurl(
		`/global/memberships${params}&status=trashed`,
		"GET",
		"no-cache"
	);
	return res;
...
const NFAMembershipsTrashedIndex = async ({ params, searchParams }) => {
	...
	const activateIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/memberships/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default NFAMembershipsTrashedIndex;
