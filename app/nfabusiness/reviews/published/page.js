...
async function getReviews(params) {
	const res = await fetchurl(
		`/global/comments${params}&status=published&postType=review`,
		"GET",
		"no-cache"
	);
	return res;
...
const NFAReviewsPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/reviews/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default NFAReviewsPublishedIndex;
