...
async function getReviews(params) {
	const res = await fetchurl(
		`/global/comments${params}&status=scheduled&postType=review`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminReviewsScheduledIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/reviews/scheduled?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminReviewsScheduledIndex;
