...
async function getWeapons(params) {
	const res = await fetchurl(
		`/noadmin/weapons${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const NFAWeaponsPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/nfabusiness/weapons/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default NFAWeaponsPublishedIndex;
