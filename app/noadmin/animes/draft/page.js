...
async function getPlaylists(params) {
	const res = await fetchurl(
		`/global/playlists${params}&onairtype=anime&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminAnimesDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		"use server";
		// const rawFormData = {}
		await fetchurl(`/global/playlists/${id}/draftit`, "PUT", "no-cache");
		revalidatePath(
			`/noadmin/animes/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminAnimesDraftIndex;
