...
async function getCategories(params) {
	const res = await fetchurl(
		`/global/categories${params}&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCategoriesDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/categories/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
	...
	return (
		<>
			...
			<div className="row">
				<div className="col">
					<CreateCategoryForm
						currentpage={`/noadmin/categories/draft?page=${page}&limit=${limit}&sort=${sort}`}
						objects={categories}
					/>
				</div>
...
export default AdminCategoriesDraftIndex;
