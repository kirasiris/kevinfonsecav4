...
async function getPlaylists(params) {
	const res = await fetchurl(
		`/global/playlists${params}&onairtype=cd-album&status=scheduled`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCDAlbumsScheduledIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/cdalbums/scheduled?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminCDAlbumsScheduledIndex;
