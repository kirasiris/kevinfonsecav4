...
async function getComments(params) {
	const res = await fetchurl(
		`/global/comments${params}&status=trash&postType=comment`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCommentsTrashedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/comments/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminCommentsTrashedIndex;
