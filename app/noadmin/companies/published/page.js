...
async function getCompanies(params) {
	const res = await fetchurl(
		`/companies${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCompaniesPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/companies/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminCompaniesPublishedIndex;
