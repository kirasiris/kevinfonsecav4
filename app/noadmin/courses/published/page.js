...
async function getCourses(params) {
	const res = await fetchurl(
		`/extras/stripe/courses${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminCoursesPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/courses/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
	...
	const handleDeleteAll = async () => {
		...
		// const rawFormData = {}
		await fetchurl(
			`/noadmin/courses/deleteall/permanently`,
			"DELETE",
			"no-cache"
		);
		revalidatePath(
...
export default AdminCoursesPublishedIndex;
