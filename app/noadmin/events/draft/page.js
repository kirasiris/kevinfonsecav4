...
async function getEvents(params) {
	const res = await fetchurl(
		`/global/events${params}&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminEventsDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/events/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminEventsDraftIndex;
