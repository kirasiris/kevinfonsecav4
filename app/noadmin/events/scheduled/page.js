...
async function getEvents(params) {
	const res = await fetchurl(
		`/global/events${params}&status=scheduled`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminEventsScheduledIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/events/scheduled?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminEventsScheduledIndex;
