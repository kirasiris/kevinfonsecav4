...
import List from "@/components/noadmin/forums/list";

async function getForums(params) {
	const res = await fetchurl(
		`/global/forums${params}&status=scheduled`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminForumsScheduledIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/forums/scheduled?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminForumsScheduledIndex;
