...
async function getForums(params) {
	const res = await fetchurl(
		`/global/forums${params}&status=trash`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminForumsTrashedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/forums/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminForumsTrashedIndex;
