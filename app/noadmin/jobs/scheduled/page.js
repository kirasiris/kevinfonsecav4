...
import Single from "@/components/admin/jobs/single";
...
const AdminJobsScheduledIndex = () => {
	...
	const fetchJobs = async () => {
		try {
			...
			setTotalResults({ ...totalResults, jobs: res?.countAll });
	...
	useEffect(() => {
		if (keyword !== "") {
			const result = jobs.filter((object) => {
	...
	return (
		<>
			<AdminStatusesMenu
				...
				filledLink="/noadmin/jobs/filled"
				categoriesLink=""
				categoryType=""
			/>
			<div className="card rounded-0">
				<AdminCardHeaderMenu
					...
					totalResults={totalResults.jobs}
...
export default AdminJobsScheduledIndex;
