...
async function getMembershipsPublished(params) {
	const res = await fetchurl(
		`/extras/stripe/memberships${params}&status=trashed`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminMembershipsTrashedIndex = async ({ params, searchParams }) => {
	...
	const activateIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/memberships/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
