...
async function getPlaylists(params) {
	const res = await fetchurl(
		`/playlists${params}&onairtype=movie&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminMoviesDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/movies/draft?page=${searchParams.page || 1}&limit=${
...
export default AdminMoviesDraftIndex;
