...
async function getPlaylists(params) {
	const res = await fetchurl(
		`/global/playlists${params}&onairtype=movie&status=trash`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminMoviesTrashedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/movies/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminMoviesTrashedIndex;
