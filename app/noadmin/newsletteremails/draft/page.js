...
async function getNewsletterEmails(params) {
	const res = await fetchurl(
		`/global/newsletteremails${params}&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminNewsletterEmailsDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/newsletteremails/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminNewsletterEmailsDraftIndex;
