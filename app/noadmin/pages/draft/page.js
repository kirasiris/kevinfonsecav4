...
async function getPages(params) {
	const res = await fetchurl(`/pages${params}&status=draft`, "GET", "no-cache");
	return res;
}

const AdminPagesDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/pages/draft?page=${searchParams.page || 1}&limit=${
