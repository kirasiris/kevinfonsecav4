...
const AdminPagesTrashedIndex = () => {
	...
	const fetchPages = async () => {
		try {
			const res = await fetchurl(`/pages${params}`);
			setPages(res?.data);
			setTotalPages(res?.pagination?.totalpages);
			setCurrentResults(res?.count);
			setTotalResults({ ...totalResults, pages: res?.countAll });
	...
	useEffect(() => {
		if (keyword !== "") {
			const result = pages.filter((object) => {
	...
	return (
		<>
			<AdminStatusesMenu
				...
				categoriesLink="/noadmin/pages/categories"
				categoryType=""
			/>
...
export default AdminPagesTrashedIndex;
