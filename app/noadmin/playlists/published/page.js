...
async function getPlaylists(params) {
	const res = await fetchurl(
		`/playlists${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminPlaylistsPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/playlists/published?page=${searchParams.page || 1}&limit=${
...
export default AdminPlaylistsPublishedIndex;
