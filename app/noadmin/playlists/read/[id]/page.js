...
const ReadPlaylist = async ({ params, searchParams }) => {
	const playlist = await getPlaylist(`/${params.id}`);
	const chapters = await getChapters(
		`?resourceId=${playlist?.data?._id}&sort=orderingNumber`
	);
					...
					<div className="card-header">
						...
						<div className="float-end my-1">
							<div className="btn-group">
								<Link
									href={{
										pathname: `/noadmin/playlists/chapter/${playlist?.data?._id}/create`,
										query: {},
									}}
									passHref
									legacyBehavior
					...
					{chapters?.data?.length > 0 ? (
						...
						>
							{chapters?.data?.map((chapter, index) => (
								<li
									key={chapter._id}
									className={`list-group-item ${chapter.orderingNumber}`}
									draggable
								...
								>
									...
									<div className="float-end">
										{chapter.free_preview && <PreviewModal object={chapter} />}
