...
async function getPolls(params) {
	const res = await fetchurl(`/polls${params}&status=draft`, "GET", "no-cache");
	return res;
}

const AdminPollsDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/polls/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminPollsDraftIndex;
