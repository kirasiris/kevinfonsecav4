...
async function getPolls(params) {
	const res = await fetchurl(
		`/global/polls${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminPollsPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/polls/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminPollsPublishedIndex;
