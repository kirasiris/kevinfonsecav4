...
async function getQRCodes(params) {
	const res = await fetchurl(
		`/extras/tools/qrcodes${params}&status=trash`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminQRCodesGeneratorTrashedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/qrcodes/trashed?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminQRCodesGeneratorTrashedIndex;
