...
async function getQuotes(params) {
	const res = await fetchurl(
		`/global/quotes${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminQuotesPublishedIndex = async ({ params, searchParams }) => {
	...
	const createQuote = async (formData) => {
		...
		revalidatePath(
			`/noadmin/quotes/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminQuotesPublishedIndex;
