...
async function getSecrets(params) {
	const res = await fetchurl(
		`/global/secrets${params}&status=draft`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminSecretsDraftIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/secrets/draft?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
	...
	return (
		<>
			...
			<Form
				...
				revalidateUrl={`/noadmin/secrets/draft?page=${page}&limit=${limit}&sort=${sort}`}
			/>
...
export default AdminSecretsDraftIndex;
