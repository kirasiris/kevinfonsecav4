...
import List from "@/components/noadmin/snippets/list";

async function getSnippets(params) {
	const res = await fetchurl(
		`/global/snippets${params}&status=published`,
		"GET",
		"no-cache"
	);
	return res;
...
const AdminSnippetsPublishedIndex = async ({ params, searchParams }) => {
	...
	const draftIt = async (id) => {
		...
		revalidatePath(
			`/noadmin/snippets/published?page=${page}&limit=${limit}&sort=${sort}`
		);
	};
...
export default AdminSnippetsPublishedIndex;
