...
const UpdateQuestionForm = ({
	...
	const upgradeQuestion = async (e) => {
		...
		router.push(`/noadmin/polls/read/${object?.data?.resourceId?._id}`);
	};

	return (
		<form key={object?.data?._id} className="row" onSubmit={upgradeQuestion}>
			<div className="col">
				...
				<div className="row">
					<div className="col">
						...
						<input
							id="answersA"
							name="answers"
							defaultValue={object?.data?.answers?.A?.text}
							type="text"
							className="form-control mb-3"
							placeholder=""
						/>
